...
```bash
python3 moutai_secondary_market_chart.py   --start 2018-01-01   --end 2026-12-31   --stock-csv data/stock_prices_auto.csv   --liquor-csv data/moutai_prices_0311_clean.csv   --output-png output/moutai_stock_vs_liquor_dual_axis.png   --output-html output/moutai_stock_vs_liquor_interactive.html   --output-merged-csv output/moutai_stock_vs_liquor_merged.csv
```
...
数据说明：
- 默认从 `data/moutai_prices_0311_clean.csv` 读取酒价（列固定为 `date, original_box_price, bulk_price`）。
- 若该 CSV 缺失或列名不匹配，脚本会直接报错，提示补齐真实文件。
